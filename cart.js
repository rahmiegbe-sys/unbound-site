...
function ubAddToCart(product){
  // product: {id, name, price (number or null if TBD), img, meta}
...
function ubChangeQty(id, delta){
  const cart = ubGetCart();
  const item = cart.find(i => i.id === id);
  if(!item) return;
  item.qty += delta;
  const next = item.qty < 1 ? cart.filter(i => i.id !== id) : cart;
  ubSaveCart(next);
  ubRenderCartCount();
  ubRenderCartDrawer();
  ubBumpCartIcon();
...
function ubRemoveFromCart(id){
  const cart = ubGetCart().filter(i => i.id !== id);
  ubSaveCart(cart);
  ubRenderCartCount();
  ubRenderCartDrawer();
  ubBumpCartIcon();
    ...
    <div class="cart-item">
      ...
      <div>
        ...
        <div class="qty-control">
          <button onclick="ubChangeQty('${item.id}', -1)" aria-label="Decrease quantity">–</button>
          <span>${item.qty}</span>
          <button onclick="ubChangeQty('${item.id}', 1)" aria-label="Increase quantity">+</button>
        </div>
      </div>
      <div>
        <div class="ci-price">${item.price ? ubFormatNaira(item.price * item.qty) : 'TBD'}</div>
        <button class="ci-remove" onclick="ubRemoveFromCart('${item.id}')">Remove</button>
      </div>
    </div>
  `).join('');
